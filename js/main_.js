...
// var currentURL = 'http://localhost/z_lite/public/';
...
window.addEventListener('load', function() {
      ...
      if (this.readyState == 4 && this.status == 200) {
        ...
        for (var i = 1; i <= eyewears.length; i += 6) {
          ...
          for (var j = i; j < i + 6; j++)
          {
            ...
            {
              html += '<div class="col-md-2 col-sm-2 col-xs-2" style="left: 50%; transform: translateX(-50%);"><label>' + eyewears[j - 1].ref_number + '<br>' + eyewears[j - 1].name + '</label>';
              html += '<img value="' + eyewears[j - 1]._id + '" class="';
              if (i == 1 && j == 1) {
                html += 'active';
              }
              html += '" src="' + eyewears[j - 1].images.preview + '"></div>';
            }
          }
          html += '</div>';
        }
  ...
  var url = currentURL + 'data/localDiskDb_eyewear22.json';
  // var url = currentURL+'/GlassesView/'+glasses_id;
      ...
      var waitForImageReady = function() {
        if (imageReady != true) {
          ...
          faceVTO.setGlassesImage(
            isOptical,
            path + 'frame/' + currentEyewearData.images.frame, 
            path + 'lenses/' + currentEyewearData.images.lenses,
            path + 'rleg/' + currentEyewearData.images.rleg, 
            path + 'lleg/' + currentEyewearData.images.lleg,
            path + 'shadow/' + currentEyewearData.images.shadow, 
            '../images/footshadow.png', 
            envCubeMap, 
            currentEyewearData.size
          );
          // var path = '';
          // faceVTO.setGlassesImage(
          //   isOptical,
          //   path + '' + currentEyewearData.images.frame, 
          //   path + '' + currentEyewearData.images.lenses,
          //   path + '' + currentEyewearData.images.rleg, 
          //   path + '' + currentEyewearData.images.lleg,
          //   path + '' + currentEyewearData.images.shadow, 
          //   'images/footshadow.png', 
          //   envCubeMap, 
          //   currentEyewearData.size
          // );
  ...
  checkloading("js/tracker_js_fix.js");
});
